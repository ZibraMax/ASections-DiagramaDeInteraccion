...
function calcularDeformaciones(c, b, h, dprima, varillas) {
	for (var i = 0; i < varillas.length; i++) {
		...
		varillas[i][4] =
			-(ecu * (Math.abs(h - coordY) / c - 1)) / varillas[i][3];
...
function dibujarViga() {
	...
	for (let i = dprima / presicionDeGrilla; i < limiteX + 1; i++) {
		for (var j = dprima / presicionDeGrilla; j < limiteY + 1; j++) {
			indicesMov.push(
				i * presicionDeGrilla * mult +
					margen +
					"," +
					(W - margen - j * presicionDeGrilla * mult)
			);
		}
	}
	imgDATA = ctx.getImageData(0, 0, canvas.width, canvas.height);
...
function calcularMomentoSapo() {
	let info = soyDasus(b, h, dprima, varillas);
	let msg = "";
	for (const i of info) {
		msg += i + "<br>";
	}
...
function moviendo(event) {
	...
	if (poniendoVarilla) {
		...
		ctx.fillText(
			"(" +
				(x / mult - margen / mult).toFixed(3) +
				", " +
				(Math.max(b, h) - y / mult + margen / mult).toFixed(3) +
				")",
			x - 10,
			y - 10
		);
...
function definirBarra(n) {
	...
	for (var i = 0; i < g.length; i++) {
		document
			.getElementById("" + g[i])
			.childNodes[1].classList.remove("activo");
	}
...
function saveString() {
	...
	download(
		a,
		"ASections[DI]-[" + new Date().toLocaleString() + "].ase",
		"text/ase"
	);
}
...
function cargarFigura(i) {
	...
	document.getElementById("recubrimiento").value =
		figuras[figuraActual].d * 100;
	actualizar();
...
function dibujarVarillasActuales(varillas) {
	...
	if (cuantia < 0.01 || cuantia > 0.04) {
		...
		let textoCuantia =
			"ρ = " +
			parseFloat(cuantia).toFixed(4) +
			", cuantía fuera del limite";
...
function descargarResultadosFigura(i) {
	...
	setTimeout(function () {
		didi(100, b, h, varillas, dprima);
		Plotly.downloadImage("graficas", {
			format: "png",
			width: 800,
			height: 600,
			filename: "Col_" + (i + 1) + "X+",
	...
	setTimeout(function () {
		didi(100, b, h, varillas, dprima);
		Plotly.downloadImage("graficas2", {
			format: "png",
			width: 800,
			height: 600,
			filename: "Col_" + (i + 1) + "X-",
	...
	setTimeout(function () {
		didi(100, b, h, varillas, dprima);
		Plotly.downloadImage("graficas3", {
			format: "png",
			width: 800,
			height: 600,
			filename: "Col_" + (i + 1) + "Y+",
	...
	setTimeout(function () {
		didi(100, b, h, varillas, dprima);
		Plotly.downloadImage("graficas4", {
			format: "png",
			width: 800,
			height: 600,
			filename: "Col_" + (i + 1) + "Y-",
...
function graficarPuntos() {
	...
	for (var i = 0; i < combos.length; i++) {
		...
		let trace5 = {
			y: puntosDeEvaluacion[combos[i]][1]
				.concat(puntosDeEvaluacion[combos[i]][3])
				.concat(
					puntosDeEvaluacion[combos[i]][1].concat(
						puntosDeEvaluacion[combos[i]][3]
					)
				),
			x: puntosDeEvaluacion[combos[i]][2]
				.concat(puntosDeEvaluacion[combos[i]][4])
				.concat(
					puntosDeEvaluacion[combos[i]][4].concat(
						puntosDeEvaluacion[combos[i]][2]
					)
				),
			z: puntosDeEvaluacion[combos[i]][0]
				.concat(puntosDeEvaluacion[combos[i]][0])
				.concat(
					puntosDeEvaluacion[combos[i]][0].concat(
						puntosDeEvaluacion[combos[i]][0]
					)
				),
			mode: "markers",
			type: "scatter3d",
...
function soyDasus(b, h, dprima, varillas) {
	...
	return [
		"Momento Nominal: " + (p[0] + p[1] + p[2]),
		"Momento Nominal con phi: " + (p[0] + p[1] + p[2]) * phi,
		"Phi: " + phi,
		"C: " + c4,
	];
}
